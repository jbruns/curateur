...
def _validate_scraping(section: Dict[str, Any]) -> List[str]:
    ...
    if "gamelist_integrity_threshold" in section:
        ...
        if not isinstance(threshold, (int, float)):
            ...
            errors.append(
                "scraping.gamelist_integrity_threshold must be between 0.0 and 1.0"
            )
    ...
    if "auto_favorite_threshold" in section:
        ...
        if not isinstance(threshold, (int, float)):
            ...
            errors.append(
                "scraping.auto_favorite_threshold must be between 0.0 and 1.0"
            )
...
def _validate_runtime(section: Dict[str, Any]) -> List[str]:
    ...
    if hash_algorithm not in valid_hashes:
        errors.append(
            f"runtime.hash_algorithm must be one of: {', '.join(valid_hashes)}"
        )
        ...
        if not isinstance(override, dict):
            ...
            if max_workers is not None:
                if not isinstance(max_workers, int) or not (1 <= max_workers <= 10):
                    errors.append(
                        "runtime.rate_limit_override.max_workers must be between 1 and 10"
                    )
            ...
            if rpm is not None:
                if not isinstance(rpm, int) or not (1 <= rpm <= 300):
                    errors.append(
                        "runtime.rate_limit_override.requests_per_minute must be between 1 and 300"
                    )
            ...
            if daily_quota is not None:
                if not isinstance(daily_quota, int) or daily_quota < 1:
                    errors.append(
                        "runtime.rate_limit_override.daily_quota must be a positive integer"
                    )
