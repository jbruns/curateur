...
class GamelistWriter:
    ...
    def write_gamelist(self, game_entries: List[GameEntry], output_path: Path) -> None:
        """
        ...
        """
        logger.info(
            f"Writing gamelist with {len(game_entries)} entries to: {output_path}"
        )
    ...
    def _create_game_element(self, entry: GameEntry) -> etree.Element:
        ...
        if entry.favorite:
            logger.debug(
                f"Writing favorite=true for entry: {entry.name} (path={entry.path})"
            )
    ...
    def _add_element(self, parent: etree.Element, tag: str, text: str) -> None:
        """
