...
class PerformanceMonitor:
    ...
    def __init__(self, total_roms: int):
        ...
        self.rom_times: Deque[float] = deque(
            maxlen=50
        )  # Last 50 total ROM processing times
    ...
    def record_api_call(self, duration: Optional[float] = None) -> None:
        ...
        if duration is not None and duration > 0:
            ...
            logger.debug(
                f"Recorded API call duration: {duration:.3f}s ({len(self.api_times)} samples)"
            )
    ...
    def record_rom_processing(self, duration: Optional[float] = None) -> None:
        ...
        if duration is not None and duration > 0:
            ...
            logger.debug(
                f"Recorded ROM processing duration: {duration:.3f}s ({len(self.rom_times)} samples)"
            )
    ...
    def get_metrics(self) -> PerformanceMetrics:
        ...
        if (
            ...
            or self.roms_processed - self.eta_last_calculated_at
            >= self.eta_recalc_interval
        ):
