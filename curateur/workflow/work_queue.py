...
class WorkQueueManager:
    ...
    def add_work(
        self, rom_info: dict, action: str, priority: Priority = Priority.NORMAL
    ) -> None:
        """
    ...
    async def mark_processed(self, work_item: WorkItem) -> None:
        ...
        filename = work_item.rom_info.get(
            "filename", work_item.rom_info.get("id", "unknown")
        )
    ...
    def mark_system_complete(self) -> None:
        ...
        logger.debug(
            "System marked as complete - pipeline tasks will exit after queue drains"
        )
    ...
    async def drain(self, timeout: float = 300.0) -> None:
        ...
        while not self.queue.empty():
            if time.time() - start_time > timeout:
                ...
                if timeout > 5.0:
                    logger.warning(
                        f"Queue drain timed out after {timeout}s with {self.queue.qsize()} items remaining"
                    )
                break
