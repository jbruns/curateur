...
class WorkflowEvaluator:
    ...
    def __init__(self, config: Dict[str, Any], cache=None):
        ...
        self.clean_mismatched_media = self.media_config.get(
    ...
    def _should_filter_disc(
        self, rom_info: ROMInfo, system: SystemDefinition
        ...
        base_name, disc_number = split_base_and_disc(Path(filename_to_check).stem)
    ...
    def evaluate_rom(
        ...
        if self.scrape_mode == "skip":
            if gamelist_entry is not None and rom_info.path.exists():
                ...
                logger.info(f"Skipping {rom_info.filename}: {decision.skip_reason}")
                return decision
        ...
        if self.scrape_mode == "new_only":
            if gamelist_entry is not None:
                decision.skip_reason = (
                    "scrape_mode is 'new_only'; ROM exists in gamelist"
                )
                logger.debug(f"Skipping {rom_info.filename}: {decision.skip_reason}")
                return decision
        ...
        if self.scrape_mode == "force":
            ...
            if gamelist_entry is None:
                ...
                if self.validation_mode == "disabled":
                    decision.skip_reason = (
                        "scrape_mode is 'skip' but validation_mode is 'disabled'"
                    )
                    logger.info(f"Skipping {rom_info.filename}: {decision.skip_reason}")
                    return decision
                else:
    ...
    def _check_hash_match(
        self, calculated_hash: Optional[str], gamelist_entry: Optional[GameEntry]
    ) -> bool:
        """
    ...
    def should_clean_media(self, media_type: str) -> bool:
        ...
        """
        return (
            self.clean_mismatched_media and media_type not in self.enabled_media_types
        )
