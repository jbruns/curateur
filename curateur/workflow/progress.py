...
class ProgressTracker:
    ...
    def log_rom(self, rom_name: str, status: str, detail: str = "") -> None:
        ...
        # Store in history
        self.current_system.roms.append(
            ROMProgress(name=rom_name, status=status, detail=detail)
        )
    ...
    def print_final_summary(self) -> None:
        ...
        if len(self.systems) > 1:
            ...
            for system in self.systems:
                success_rate = (
                    (system.succeeded / system.total_roms * 100)
                    if system.total_roms > 0
                    else 0
                )
                print(
                    f"  {system.name}: {system.succeeded}/{system.total_roms} ({success_rate:.1f}%)"
                )
            print()
