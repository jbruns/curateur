...
class WorkflowOrchestrator:
    ...
    def __init__(
        ...
        self.evaluator = WorkflowEvaluator(
            self.config, cache=self.api_client.cache if self.api_client else None
        )

        # Initialize integrity validator
        integrity_threshold = self.config.get("scraping", {}).get(
    ...
    def update_search_config(
        ...
        if confidence_threshold is not None:
            ...
            logger.info(
                f"Updated search confidence threshold to {confidence_threshold:.0%}"
            )
    ...
    async def scrape_system(
        ...
        cache = MetadataCache(
            gamelist_directory=gamelist_dir, ttl_days=7, enabled=enable_cache
        )
        ...
        if self.clear_cache:
            if enable_cache:
                ...
                logger.info(
                    f"Cleared metadata cache: {cleared_count} entries removed, building new cache"
                )
            else:
        ...
        crc_size_limit = self.config.get("runtime", {}).get(
            "crc_size_limit", 1073741824
        )
        rom_entries = scan_system(
            system, rom_root=self.rom_directory, crc_size_limit=crc_size_limit
        )
        ...
        if len(rom_entries) == 0:
            logger.info(
                f"No ROMs found for {system.name}, skipping all filesystem operations"
            )
            ...
            return SystemResult(
                ...
                results=[],
                work_queue_stats=(
                    self.work_queue.get_stats() if self.work_queue else None
                ),
                failed_items=(
                    self.work_queue.get_failed_items() if self.work_queue else None
                ),
                not_found_items=[],
            )
        ...
        if gamelist_path.exists():
            ...
            try:
                ...
                logger.info(
                    f"Parsed {len(existing_entries)} entries from existing gamelist"
                )
                ...
                validation_result = self.integrity_validator.validate(
                    existing_entries, rom_paths
                )

                logger.info(
                    f"Gamelist validation: {validation_result.match_ratio:.1%} match ratio"
                )
                ...
                if not validation_result.is_valid:
                    ...
                    if not self._prompt_gamelist_validation_failure(
                        system.name, validation_result
                    ):
                        logger.info(
                            f"Skipping system {system.name} due to validation failure"
                        )
                        return SystemResult(
                            system_name=system.fullname,
                            total_roms=len(rom_entries),
        ...
        results, not_found_items = await self._scrape_roms_parallel(
            system, rom_entries, media_types, preferred_regions, existing_entries
        )
        ...
        if not self.dry_run and (scraped_count > 0 or existing_entries):
            try:
                ...
                logger.debug(
                    f"About to call _generate_gamelist with {len(results)} results"
                )
        ...
        if (
            len(rom_entries) > 0
            and system.name in self.unmatched_roms
            and self.unmatched_roms[system.name]
        ):
            try:
                self._write_unmatched_roms(system.name)
            except Exception as e:
                logger.warning(
                    f"Failed to write unmatched ROMs log for {system.name}: {e}"
                )
        ...
        if len(rom_entries) > 0 and not_found_items:
            try:
                ...
                logger.warning(
                    f"Failed to write not-found summary for {system.name}: {e}"
                )
        ...
        if len(rom_entries) > 0:
            self._write_summary_log(
                system, results, scraped_count, skipped_count, failed_count
            )
    ...
    async def _scrape_rom(
        ...
        try:
            ...
            if not rom_hash:
                logger.debug(
                    f"[{rom_info.filename}] No hash value available (skipped or failed during batch hashing)"
                )
                completed_tasks += 1
            else:
                logger.debug(
                    f"[{rom_info.filename}] Using pre-calculated hash: {rom_hash}"
                )
                completed_tasks += 1
            ...
            if self.dry_run:
                return ScrapingResult(
                    rom_path=rom_info.path, success=True, api_id="DRY_RUN"
                )
            ...
            if decision.fetch_metadata:
                ...
                if self.api_client.cache:
                    cached_data = self.api_client.cache.get(
                        rom_hash, rom_size=rom_info.file_size
                    )
                ...
                try:
                    game_info = await self.api_client.query_game(
                        rom_info, shutdown_event=shutdown_event
                    )
                    ...
                    if (
                        game_info
                        and "descriptions" in game_info
                        and "desc" not in game_info
                    ):
                        descriptions = game_info["descriptions"]
                        preferred_lang = self.config.get("scraping", {}).get(
                    ...
                    if game_info:
                        ...
                        field_count = len(
                            [k for k in game_info.keys() if game_info.get(k)]
                        )
                        logger.info(
                            f"[{rom_info.filename}] "
                        ...
                        if self.event_bus:
                            ...
                            release_date_str = (
                                release_dates.get("us")
                                or release_dates.get("wor")
                                or release_dates.get("eu")
                                or None
                            )
                            year = (
                                release_date_str[:4]
                            ...
                            genre = (
                                ", ".join(genres[:2]) if genres else "Unknown"
                            )  # Limit to 2 genres for display
                            ...
                            await self.event_bus.publish(
                                GameCompletedEvent(
                                    ...
                                    description=game_info.get("desc", "")[
                                        :300
                    ...
                    logger.info(
                        f"[{rom_info.filename}] API request cancelled (shutdown)"
                    )
                    return ScrapingResult(
                        rom_path=rom_info.path,
                        success=False,
                    ...
                    if self.enable_search_fallback:
                        ...
                        game_info = await self._search_fallback(
                            rom_info, preferred_regions, shutdown_event=shutdown_event
                        )
                        ...
                        if game_info:
                            ...
                            logger.info(
                                f"[{rom_info.filename}] Search fallback successful"
                            )
                            ...
                            logger.info(
                                f"[{rom_info.filename}] Search fallback: no matches found"
                            )
                    else:
                        raise
            ...
            media_count = 0
            media_hashes = {}
            hash_algorithm = self.config.get("runtime", {}).get(
            ...
            if game_info:
                ...
                media_semaphore = (
                    self.throttle_manager.media_download_semaphore
                ...
                if decision.media_to_download and media_list:
                    ...
                    rom_basename = media_downloader.organizer.get_rom_basename(
                        str(rom_info.path)
                    )
                    existing_media = {}
                    ...
                    for media_type_singular in decision.media_to_download:
                        ...
                        screenscraper_types = convert_directory_names_to_media_types(
                            [plural_dir]
                        )
                        ...
                        if screenscraper_types:
                            ...
                            for ext in ["jpg", "png", "gif", "webp", "mp4", "pdf"]:
                                media_path = media_downloader.organizer.get_media_path(
                                    system.name, screenscraper_type, rom_basename, ext
                                )
                                if media_downloader.organizer.file_exists(media_path):
                                    existing_media[media_type_singular] = True
                                    existing_media_paths[media_type_singular] = Path(
                                        media_path
                                    )
                                    logger.debug(
                    ...
                    if validation_mode == "disabled" and existing_media:
                        ...
                        for media_type_singular in existing_media:
                            if existing_media[media_type_singular]:
                                media_path = existing_media_paths.get(
                                    media_type_singular
                                )
                        ...
                        for media_type_singular in existing_media:
                            ...
                            is_image_type = media_type_singular not in [
                                "manual",
                                "video",
                            ]
                            ...
                            if validation_mode == "strict":
                                ...
                                if is_image_type:
                                    is_valid, validation_error = (
                                        media_downloader.downloader.validate_existing_file(
                                            media_path
                                        )
                                    )
                                    if not is_valid:
                                ...
                                current_hash = calculate_hash(
                                    media_path, algorithm=hash_algorithm, size_limit=0
                                )
                            ...
                            if validation_passed:
                                ...
                                if (
                                    media_type_singular
                                    not in self.session_stats["media_by_type"]
                                ):
                                    self.session_stats["media_by_type"][
                                        media_type_singular
                                    ] = {
                                        "successful": 0,
                                        "failed": 0,
                    ...
                    media_types_to_download = [
                        mt
                        for mt in decision.media_to_download
                        if mt not in validated_media
                    ]

                    if not media_types_to_download:
                        logger.info(
                            f"[{rom_info.filename}] All media already exists on disk, skipping downloads"
                        )
                        # Clear download list and skip all download logic
                        decision.media_to_download = []
                    else:
                        if len(media_types_to_download) < len(
                            decision.media_to_download
                        ):
                            skipped = len(decision.media_to_download) - len(
                                media_types_to_download
                            )
                            logger.info(
                        ...
                        unavailable_types = [
                            mt
                            for mt, ss_type in zip(
                                decision.media_to_download, screenscraper_types
                            )
                        ...
                        if filtered_media_list:
                            ...
                            def media_progress_callback(
                                media_type: str, current_idx: int, total_count: int
                            ):
                                pass
                            ...
                            for result in download_results:
                                if result.success and result.file_path:
                                    ...
                                    plural_dir = get_directory_for_media_type(
                                        result.media_type
                                    )
                                    ...
                                    if result.hash_value:
                                        media_hashes[media_type_singular] = (
                                            result.hash_value
                                        )
                                        logger.debug(
                                    ...
                                    failed_downloads.append(
                                        (media_type_singular, result.error)
                                    )
                    ...
                    if decision.media_to_validate and validation_mode != "disabled":
                        ...
                        for media_type_singular in decision.media_to_validate:
                            ...
                            media_path = self._get_media_path(
                                system, media_type_singular, rom_info.path
                            )
                            if not media_path or not media_path.exists():
                                ...
                                if (
                                    media_type_singular
                                    not in decision.media_to_download
                                ):
                                    decision.media_to_download.append(
                                        media_type_singular
                                    )
                                continue
                            ...
                            if self.api_client.cache and rom_hash:
                                expected_hash = self.api_client.cache.get_media_hash(
                                    rom_hash, media_type_singular
                                )
                            ...
                            if validation_mode == "strict":
                                ...
                                if is_image_type:
                                    ...
                                    is_valid, validation_error = (
                                        temp_downloader.validate_existing_file(
                                            media_path
                                        )
                                    )
                                    if is_valid:
                                        ...
                                        validated_trusted.append(media_type_singular)
                                        media_paths[media_type_singular] = str(
                                            media_path
                                        )
                                        media_hashes[media_type_singular] = (
                                            expected_hash
                        ...
                        if (
                            validated_passed
                            or validated_failed
                            or validated_missing
                            or validated_no_hash
                        ):
                            summary_parts = []
                            if validated_passed:
                                summary_parts.append(
                            ...
                            if validated_failed:
                                failed_details = [
                                    f"{mt}" for mt, exp, got in validated_failed
                                ]
                                summary_parts.append(
                    ...
                    if decision.media_to_download:
                        ...
                        if redownload_media_list:
                            ...
                            available_redownload_types = {
                                m.get("type") for m in redownload_media_list
                            }
                            actual_redownload_count = len(
                                [
                                    mt
                                    for mt in decision.media_to_download
                                    if any(
                                        st in available_redownload_types
            ...
            if decision.clean_disabled_media:
                ...
                for media_type_singular in disabled_types:
                    media_path = self._get_media_path(
                        system, rom_info, media_type_singular
                    )
            ...
            if decision.update_metadata and game_info:
                ...
                if gamelist_entry and decision.update_metadata:
                    merge_result = self.metadata_merger.merge_entries(
                        gamelist_entry, game_entry
                    )
                    ...
                    if self.metadata_merger.auto_favorite_enabled:
                        if (
                            game_entry.rating is not None
                            and game_entry.rating
                            >= self.metadata_merger.auto_favorite_threshold
                        ):
                            game_entry.favorite = True
                            logger.debug(
                                f"Auto-favoriting new entry: {game_entry.name} (rating={game_entry.rating})"
                            )
                ...
                if self.api_client.cache and rom_hash and media_hashes:
                    ...
                    logger.debug(
                        f"Updated cache with {len(media_hashes)} media hashes for {rom_info.filename}"
                    )
            ...
            return ScrapingResult(
                ...
                media_paths=media_paths,
                game_entry=(
                    game_entry if decision.update_metadata and game_info else None
                ),
            )
            ...
            return ScrapingResult(rom_path=rom_info.path, success=False, error=str(e))
    ...
    def _create_rom_processor(
        ...
        async def process_rom(
            ...
            try:
                ...
                return ScrapingResult(
                    rom_path=rom_info.path, success=False, error=str(e)
                )
        ...
        for i in range(0, total, batch_size):
            ...
            for rom_info in batch:
                ...
                if scrape_mode == "new_only" and existing_entries:
                    rom_relative_path = f"./{rom_info.filename}"
                    if any(
                        entry.path == rom_relative_path for entry in existing_entries
                    ):
                        logger.debug(
                            f"Skipping hash for existing ROM in new_only mode: {rom_info.filename}"
                        )
                        continue
                ...
                if rom_info.rom_type == ROMType.STANDARD:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to get disc1 file for {rom_info.name}: {e}"
                        )
                        continue
                    ...
                    if rom_info.contained_file:
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to get contained file for {rom_info.name}: {e}"
                            )
                            continue
                else:
                    hash_file = rom_info.path
            ...
            if hash_tasks:
                ...
                large_files = [
                    rom_info
                    for rom_info, _ in hash_tasks
                    if rom_info.file_size > 100_000_000
                ]  # > 100MB
                if large_files:
                    largest_gb = max(rom_info.file_size for rom_info in large_files) / (
                        1024**3
                    )
                    logger.info(
                ...
                results = await asyncio.gather(
                    *[task for _, task in hash_tasks], return_exceptions=True
                )
            ...
            should_log = (
                (current_count % 10 == 0)  # Every 10 ROMs
                or (current_time - last_log_time >= log_interval)  # Every 10 seconds
                or (current_count >= total)  # Always log completion
            )
    ...
    async def _scrape_roms_parallel(
        ...
        for idx, rom_info in enumerate(rom_entries):
            try:
                ...
                rom_info_dict = {
                    ...
                    "disc_files": (
                        [str(f) for f in rom_info.disc_files]
                        if rom_info.disc_files
                        else None
                    ),
                    "contained_file": (
                        str(rom_info.contained_file)
                        if rom_info.contained_file
                        else None
                    ),
                }
                ...
                logger.warning(
                    f"Keyboard interrupt received while adding ROM {idx} to queue"
                )
                raise
                ...
                logger.error(
                    f"Failed to add ROM {idx} ({rom_info.filename}) to work queue: {e}",
                    exc_info=True,
                )
                raise
        ...
        if (
            self.thread_manager
            and self.thread_manager.is_initialized()
            and not self.dry_run
        ):
            logger.info(
                f"Using task pool with {self.thread_manager.max_concurrent} concurrent task(s)"
            )

            # Create ROM processor (no UI callback needed for pipeline UI)
            rom_processor = self._create_rom_processor(
                system, media_types, preferred_regions, existing_entries
            )
            ...
            for result in results_by_path.values():
                ...
                if not result.success and result.error == "No game info found from API":
                    not_found_items.append(
                        {"filename": result.rom_path.name, "path": str(result.rom_path)}
                    )
            ...
            for rom_info in rom_entries:
                ...
                if self.textual_ui and self.textual_ui.should_quit:
                    logger.info(
                        "Quit requested from Textual UI during sequential processing"
                    )
                    break
                ...
                if self.textual_ui and self.textual_ui.should_skip_system:
                    logger.info(
                        "Skip system requested from Textual UI during sequential processing"
                    )
                    break
                ...
                try:
                    ...
                    if (
                        not result.success
                        and result.error == "No game info found from API"
                    ):
                        not_found_items.append(
                            {"filename": rom_info.filename, "path": str(rom_info.path)}
                        )
        ...
        try:
            ...
            for game_data in results:
                confidence = calculate_match_confidence(
                    rom_info_dict, game_data, preferred_regions
                )
            ...
            if self.interactive_search:
                if self.textual_ui:
                    ...
                    selected_game = await self._wait_for_search_response(
                        request_id, rom_info, scored_candidates
                    )

                    if selected_game:
                        game_name = selected_game.get("names", {}).get("en", "Unknown")
                        logger.info(f"[{rom_info.filename}] User selected: {game_name}")
                        return selected_game
                    else:
                        logger.info(
                            f"[{rom_info.filename}] User skipped/cancelled search"
                        )
                        return None
                else:
            ...
            logger.error(
                f"[{rom_info.filename}] Unexpected error in search fallback: {e}"
            )
            return None

    async def _wait_for_search_response(
        self, request_id: str, rom_info: ROMInfo, scored_candidates: list
    ) -> Optional[Dict]:
        ...
        search_results = [
            {"game_data": game_data, "confidence": confidence}
        ...
        if self.event_bus:
            ...
            logger.debug(
                f"[{rom_info.filename}] Emitted SearchRequestEvent, waiting for user response..."
            )
    ...
    async def handle_search_response(self, response) -> None:
        ...
        async with self.search_response_lock:
            if response.request_id in self.search_response_queues:
                ...
                logger.debug(
                    f"Search response delivered for request {response.request_id}"
                )
            else:
                logger.warning(
                    f"Received response for unknown request_id: {response.request_id}"
                )

    def _generate_gamelist(
        self, system: SystemDefinition, results: List[ScrapingResult]
    ) -> Optional[dict]:
        """
        ...
        if scraped_games or gamelist_path.exists():
            try:
                integrity_result = generator.generate_gamelist(
                    scraped_games=scraped_games, merge_existing=True
                )
    ...
    def _write_not_found_summary(
        self, system: SystemDefinition, not_found_items: List[dict]
    ) -> None:
        """
        ...
        try:
            ...
            with open(output_file, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"# Total ROMs not found in ScreenScraper: {len(not_found_items)}\n"
                )
                f.write("#\n")
                ...
                for item in sorted(
                    not_found_items, key=lambda x: x["rom_info"].filename.lower()
                ):
                    ...
                    if rom_info.hash_value:
                        f.write(
                            f"  {rom_info.hash_type.upper()}: {rom_info.hash_value}\n"
                        )
    ...
    def _prompt_gamelist_validation_failure(
        self, system_name: str, validation_result
    ) -> bool:
        """
    ...
    def _get_media_path(
        self, system: SystemDefinition, rom_info: ROMInfo, media_type_singular: str
    ) -> Optional[Path]:
        """
            ...
            with open(summary_path, "w") as f:
                ...
                if successful_results:
                    ...
                    for result in sorted(
                        successful_results, key=lambda r: r.rom_path.name.lower()
                    ):
                ...
                # Skipped results
                skipped_results = [
                    r for r in results if hasattr(r, "skipped") and r.skipped
                ]
                if skipped_results:
                    f.write("=== Skipped ===\n")
                    for result in sorted(
                        skipped_results, key=lambda r: r.rom_path.name.lower()
                    ):
                ...
                if failed_results:
                    f.write("=== Failed ===\n")
                    for result in sorted(
                        failed_results, key=lambda r: r.rom_path.name.lower()
                    ):
