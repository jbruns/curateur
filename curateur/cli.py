...
def _setup_logging(
    config: dict, console_ui=None, textual_ui=None, event_bus=None
) -> None:
    """
    ...
    if log_file:
        try:
            ...
            print(
                f"Error: Could not create log file '{log_file}': {e}", file=sys.stderr
            )
            sys.exit(1)
...
async def run_scraper(config: dict, args: argparse.Namespace) -> int:
    ...
    if systems_to_scrape:
        ...
        if not systems:
            print(
                f"Error: No matching systems found for: {systems_to_scrape}",
                file=sys.stderr,
            )
            return 1
    else:
    ...
    if not isinstance(max_retries, int) or max_retries < 1 or max_retries > 10:
        logger.warning(
            f"Invalid api.max_retries value: {max_retries}, using default: 3"
        )
        max_retries = 3
    ...
    throttle_manager = ThrottleManager(
        default_limit=RateLimit(calls=default_rpm, window_seconds=60), adaptive=True
    )
    ...
    thread_manager = ThreadPoolManager(
        config, console_ui=headless_logger, textual_ui=textual_ui
    )
    ...
    if event_bus:
        await event_bus.publish(
            AuthenticationEvent(status="authenticating", username=None)
        )
    ...
    try:
        ...
        if event_bus:
            await event_bus.publish(
                AuthenticationEvent(
                    status="authenticated", username=user_limits.get("id")
                )
            )
        ...
        logger.debug(
            f"Throttle manager initialized with quota: "
            f"{user_limits.get('requeststoday', 0)}/{user_limits.get('maxrequestsperday', 0)}"
        )

        # Initialize thread pool with actual API limits
        thread_manager.initialize_pools(user_limits)
        logger.debug(
            f"Thread pool initialized with {thread_manager.max_concurrent} concurrent tasks"
        )
        ...
        if "maxthreads" in user_limits:
            ...
            logger.info(
                f"Scaled connection pool to {pool_size} connections (aligned to API concurrency limit)"
            )
    ...
    performance_monitor = (
        PerformanceMonitor(total_roms=total_roms) if total_roms > 0 else None
    )
    ...
    orchestrator = WorkflowOrchestrator(
        ...
        preferred_regions=config["scraping"].get(
    ...
    try:
        ...
        for system in systems:
            try:
                ...
                if headless_logger and not textual_ui:
                    for rom_result in result.results:
                        if rom_result.success:
                            ...
                            progress.log_rom(
                                rom_result.rom_path.name, "success", media_info
                            )
                        elif rom_result.error:
                            progress.log_rom(
                                rom_result.rom_path.name, "failed", rom_result.error
                            )
                            error_logger.log_error(
                                rom_result.rom_path.name, rom_result.error
                            )
                        else:
                            progress.log_rom(rom_result.rom_path.name, "skipped", "")
                ...
                logger.info(
                    "Interrupted by user, stopping pipeline tasks gracefully..."
                )
                if thread_manager:
                    await thread_manager.stop_workers()
                ...
                if work_queue:
                    ...
                    if queue_stats["pending"] > 0:
                        logger.warning(
                            f"Work queue has {queue_stats['pending']} pending items"
                        )

                    if failed_items:
                        logger.warning(
                            f"Work queue has {len(failed_items)} failed items (retries exhausted):"
                        )
                        for item in failed_items[:10]:  # Log first 10
                            logger.warning(
                                f"  - {item['rom_info'].get('filename', 'unknown')}: {item['error']}"
                            )

                raise
            except Exception as e:
                logger.error(
                    f"Error processing system {system.fullname}: {e}", exc_info=True
                )
        ...
        if textual_ui_task and not textual_ui_task.done():
            ...
            logger.debug(
                f"UI task state: done={textual_ui_task.done()}, cancelled={textual_ui_task.cancelled()}"
            )
            try:
                # Call shutdown which will call exit()
                await textual_ui.shutdown()
                logger.debug(
                    "Shutdown method completed, waiting for UI task to finish..."
                )
                ...
                try:
                    ...
                    logger.warning(
                        "UI task did not complete within timeout, forcing cancellation"
                    )
        ...
        if work_queue:
            ...
            if failed_items:
                ...
                for item in failed_items[:10]:  # Show first 10
                    ...
                    error = item["error"]
                    print(
                        f"    - {rom_name} ({action}): {retry_count} retries - {error}"
                    )
