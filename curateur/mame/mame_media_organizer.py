...
class MAMEMediaOrganizer:
    ...
    def organize_media(
        ...
        if self.multimedia_path:
            logger.info(f"  videos: {stats.videos_copied} copied, {stats.videos_skipped} skipped")
    ...
    def _copy_videos(self, shortnames: Set[str], stats: MediaStats):
        ...
        if not self.multimedia_path.exists():
            logger.warning(f"MAME Multimedia path does not exist: {self.multimedia_path}")
            return
        ...
        for shortname in sorted(shortnames):
            ...
            if target_video.exists():
                ...
                if (source_stat.st_size == target_stat.st_size and
                    abs(source_stat.st_mtime - target_stat.st_mtime) < 2):
