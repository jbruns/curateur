...
class MediaExtractor:
    ...
    def _scan_archives(self):
        ...
        logger.info(f"Found {len(self.available_archives)} of {len(MAME_MEDIA_MAPPING)} media archives")
    ...
    def _extract_from_archive(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                logger.debug(f"Extracting {archive_path.name} to temporary directory...")
    ...
    def _find_and_move_media(
        ...
        if len(candidates) > 1:
            logger.debug(f"Multiple files found for {shortname}, using {candidates[0][0].name}")
        ...
        if dest_file.exists():
            ...
            if (source_stat.st_size == dest_stat.st_size and
                abs(source_stat.st_mtime - dest_stat.st_mtime) < 2):
