...
class MAMEMetadataBuilder:
    ...
    def build_game_entries(
        self, config: MAMEMetadataBuilderConfig
    ) -> Dict[str, GameEntry]:
    ...
    def _filter_machines(
        ...
        candidates = {
            name: machine for name, machine in all_machines.items() if machine.is_game()
        }
        ...
        if config.minimum_rating is not None:
            ...
            logger.info(
                f"After rating filter (>={config.minimum_rating}): {len(candidates)} games"
            )
    ...
    def _build_base_entry(
        self, machine: MAMEMachine, auto_sortname_enabled: bool = False
    ) -> Optional[GameEntry]:
        ...
        sortname = (
            GameEntry._generate_sortname(machine.description)
            if auto_sortname_enabled
            else None
        )
    ...
    def _apply_clone_inheritance(
        self, entries: Dict[str, GameEntry], machines: Dict[str, MAMEMachine]
    ):
        ...
        if missing_parents:
            logger.warning(
                f"Skipped inheritance for {len(missing_parents)} clones (parent not in scope)"
            )
    ...
    def _apply_favorite_threshold(
        self, entries: Dict[str, GameEntry], threshold: float
    ):
        ...
        if favorites_set:
            logger.info(
                f"Marked {favorites_set} games as favorites (rating >= {threshold})"
            )
