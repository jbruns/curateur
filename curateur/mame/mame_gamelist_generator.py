...
class MAMEGamelistGenerator:
    ...
    def generate(self) -> bool:
        ...
        try:
            ...
            copy_success = self._copy_roms_and_chds(
                shortnames, parsed_data["mame_parser"]
            )
            if not copy_success:
                return False
    ...
    def _copy_roms_and_chds(self, shortnames: set, mame_parser: MAMEXMLParser) -> bool:
        ...
        stats, errors = copier.copy_roms_and_chds(
            shortnames=shortnames, dry_run=self.config.dry_run
        )
    ...
    def _organize_media(self, shortnames: set):
        ...
        organizer.organize_media(shortnames=shortnames, dry_run=self.config.dry_run)
    ...
    def _generate_gamelist(self, game_entries: Dict[str, GameEntry]) -> bool:
        ...
        try:
            ...
            if self.config.gamelist_output_path.exists():
                logger.info(
                    f"Loading existing gamelist: {self.config.gamelist_output_path}"
                )
                parser = GamelistParser()
                try:
                    existing_entries = parser.parse_gamelist(
                        self.config.gamelist_output_path
                    )
            ...
            if self.config.dry_run:
                logger.info(
                    f"Dry run: would generate gamelist with {len(entries_list)} entries"
                )
                return True
            ...
            if self.config.validate_output:
                ...
                if not validation_result.is_valid:
                    ...
                    logger.info(
                        f"Gamelist validation passed (match ratio: {validation_result.match_ratio:.1%})"
                    )
