...
class MediaURLSelector:
    ...
    def select_media_urls(
        self, media_list: List[Dict], rom_filename: str
    ) -> Dict[str, Dict]:
        """
        ...
        for media_type in self.enabled_media_types:
            ...
            if should_use_region_filtering(media_type):
                best_region = select_best_region(
                    available_regions, rom_filename, self.preferred_regions
                )
                logger.debug(
                    f"  {media_type}: selected region = {best_region} (region filtering enabled)"
                )
            else:
                # No region filtering - use first available
                best_region = None
                logger.debug(
                    f"  {media_type}: no region filtering, using first available"
                )
            ...
            if media_info:
                ...
                logger.debug(
                    f"  {media_type}: selected media with region={media_info.get('region', 'N/A')}"
                )
            else:
                logger.debug(f"  {media_type}: no media found for region {best_region}")

        logger.debug(
            f"Final selection: {len(selected_media)} media types: {list(selected_media.keys())}"
        )
        return selected_media

    def _get_available_regions(
        self, media_list: List[Dict], media_type: str
    ) -> List[str]:
        """
