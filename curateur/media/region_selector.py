...
def select_best_region(
    ...
    logger.debug(
        f"    select_best_region: ROM regions = {rom_regions}, preferred = {preferred_regions}"
    )
    ...
    for region in rom_regions:
        if region in available_regions:
            logger.debug(
                f"    select_best_region: matched ROM region '{region}' (not in preferred list)"
            )
            return region
...
def get_media_for_region(
    media_list: List[Dict], media_type: str, region: Optional[str] = None
) -> Optional[Dict]:
    """
