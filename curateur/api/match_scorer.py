...
def _score_filename_similarity(rom_info: Dict[str, Any], game_data: Dict[str, Any]) -> float:
    """
    ...
    for region, name in game_names.items():
        ...
        if similarity > best_similarity:
            ...
            logger.debug(f"Filename match: '{rom_name_clean}' vs '{name_clean}' [{region}] = {similarity:.2f}")
    
    return best_similarity


def _score_region_match(game_data: Dict[str, Any], preferred_regions: list[str]) -> float:
    """
