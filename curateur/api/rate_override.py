...
class RateLimitOverride:
    ...
    def __init__(self, config: dict):
        ...
        self.custom_limits = config.get("runtime", {}).get("rate_limit_override", {})
    ...
    def get_effective_limits(
        self, api_provided_limits: Optional[Dict[str, Any]]
    ) -> RateLimits:
        """
        ...
        if self.override_enabled:
            if "max_threads" in self.custom_limits:
                ...
                if api_max_threads is not None:
                    ...
                    logger.info(
                        f"Using override max_threads: {override_threads} (no API limit available)"
                    )
                    max_threads = override_threads
            ...
            if "requests_per_minute" in self.custom_limits:
                ...
                if api_max_rpm is not None:
                    ...
                    logger.info(
                        f"Using override requests_per_minute: {override_rpm} (no API limit available)"
                    )
                    requests_per_minute = override_rpm
            ...
            if "daily_quota" in self.custom_limits:
                ...
                if api_max_daily is not None:
                    ...
                    logger.info(
                        f"Using override daily_quota: {override_quota} (no API limit available)"
                    )
                    daily_quota = override_quota
    ...
    def validate_overrides(self) -> None:
        ...
        if "daily_quota" in self.custom_limits:
            ...
            if quota < 1:
                warnings.append(f"daily_quota={quota} is invalid. Must be at least 1.")
