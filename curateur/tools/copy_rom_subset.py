...
"""
    ...
    parser.add_argument(
        'source_roms',
        type=Path,
        help='Source ROM directory (pre-filtered subset)'
    )
    parser.add_argument(
        'source_media',
        type=Path,
        help='Source media directory'
    )
    parser.add_argument(
        'target_media',
        type=Path,
        help='Target media directory'
    )
    parser.add_argument(
        'source_gamelists',
        type=Path,
        help='Source gamelist directory'
    )
    parser.add_argument(
        'target_gamelists',
        type=Path,
        help='Target gamelist directory'
    )
    parser.add_argument(
        '--es-systems',
        type=Path,
        required=True,
        help='Path to es_systems.cfg file'
    )
    parser.add_argument(
        '--systems',
        nargs='+',
    ...
    for entry in system_dir.iterdir():
        ...
        if entry.is_file():
            ...
            if is_disc_subdirectory(entry, system.extensions):
                ...
                logger.debug(f"Found disc subdir: {entry.name} -> basename: {entry.name}")
...
def copy_gamelist_and_cache(
    system: str,
    source_gamelists: Path,
    target_gamelists: Path,
    dry_run: bool
) -> tuple[bool, bool]:
    """
