            ...
            for line_num, line in enumerate(f, 1):
                ...
                if '\\"' in line:
                    ...
                    if '\\"' in code_part:
                        ...
                        if (
                            re.search(r'f\\"', code_part)
                            or re.search(r'^\s+\\"', code_part)
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if (
                ...
                if line_num < len(lines):
                    ...
                    if (
                        next_line
                        and not next_line.startswith(("def ", "class ", "@", "#"))
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            for line_num, line in enumerate(f, 1):
                ...
                if "print(" in line and "file=" not in line:
                    ...
                    if not any(
                        x in str(file_path) for x in ["cli.py", "test_", "conftest.py"]
                    ):
...
def scan_directory(
    root_path: Path, pattern: str = "**/*.py", ci_mode: bool = False
...
def main():
    ...
    if args.strict or args.ci:
        ...
        has_critical = any(
            bool(issues.get("syntax_errors")) for issues in results.values()
        )
