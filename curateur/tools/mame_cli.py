...
def validate_config(config: dict) -> list:
    ...
    if "favorite_threshold" in config and config["favorite_threshold"] is not None:
        ...
        if not isinstance(rating, (int, float)) or rating < 0.0 or rating > 1.0:
            errors.append(
                f"favorite_threshold must be between 0.0 and 1.0, got: {rating}"
            )
...
def config_to_mame_config(config: dict) -> MAMEConfig:
    ...
    return MAMEConfig(
        ...
        source_chd_path=(
            Path(config["source_chd_path"]).expanduser()
            if config.get("source_chd_path")
            else None
        ),
        mame_xml_path=Path(config["mame_xml_path"]).expanduser(),
        multimedia_path=(
            Path(config["multimedia_path"]).expanduser()
            if config.get("multimedia_path")
            else None
        ),
        ...
        inclusion_list_path=(
            ...
            if config.get("inclusion_list_path")
            else None
        ),
