...
class SystemReport:
    ...
    def add_issue(self, category: str, file_path: Path, message: str, severity: str = "warning"):
...
def validate_system(system: SystemDefinition, rom_root: Path, verbose: bool = False) -> SystemReport:
    """
    ...
    if compressed_files and uncompressed_files:
        ...
        top_level_compressed = [f for f in compressed_files if f.parent == report.rom_path]
        top_level_uncompressed = [f for f in uncompressed_files if f.parent == report.rom_path]
...
def print_report(report: ValidationReport, verbose: bool = False):
    ...
    print(f"Systems with issues: {report.systems_with_issues()} / {len(report.system_reports)}")
