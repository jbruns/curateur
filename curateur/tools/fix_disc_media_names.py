...
def find_media_files_needing_rename(
    media_root: Path, system: str, disc_subdir_name: str, verbose: bool = False
    ...
    if media_root.name == system:
        ...
        if verbose:
            print(
                f"  DEBUG: media_root already includes system name, using: {system_media_dir}"
            )
    else:
    ...
    if "." in disc_subdir_name:
        ...
        if verbose:
            print(
                f"  DEBUG: Disc subdir '{disc_subdir_name}' has no extension, skipping"
            )
        return renames
    ...
    for media_type_dir in system_media_dir.iterdir():
        ...
        if verbose and matching_files:
            print(
                f"  DEBUG: Found {len(matching_files)} matching files in {media_type_dir.name}"
            )
...
Examples:
  ...
  python -m curateur.tools.fix_disc_media_names /roms/dreamcast /downloaded_media/dreamcast --dry-run
    ...
    parser.add_argument(
        "rom_dir", type=Path, help="ROM directory to scan for disc subdirectories"
    )
    ...
    for disc_dir in disc_subdirs:
        ...
        renames = find_media_files_needing_rename(
            media_root, system_name, disc_dir.name, verbose=args.verbose
        )
