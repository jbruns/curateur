...
def config():
    ...
    return {"scraping": {"systems": ["nes", "snes", "genesis"]}}
...
async def test_hashing_progress_event_handler(config, event_bus):
    ...
    event = HashingProgressEvent(completed=75, total=100, in_progress=True, skipped=5)
...
async def test_api_activity_event_handler(config, event_bus):
    ...
    event = APIActivityEvent(
        metadata_in_flight=3, metadata_total=50, search_in_flight=1, search_total=10
    )
...
async def test_log_entry_event_handler(config, event_bus):
    ...
    for level, message in levels:
        event = LogEntryEvent(level=level, message=message, timestamp=datetime.now())
...
async def test_cache_metrics_event_handler(config, event_bus):
    ...
    event = CacheMetricsEvent(
        existing=500, added=25, hits=450, misses=75, hit_rate=85.7
    )
...
async def test_gamelist_update_event_handler(config, event_bus):
    ...
    event = GamelistUpdateEvent(system="nes", existing=200, added=15, updated=5)
...
async def test_search_activity_event_handler(config, event_bus):
    ...
    event = SearchActivityEvent(fallback_count=5, unmatched_count=3)
...
async def test_authentication_event_handler(config, event_bus):
    ...
    # Test authenticating
    event1 = AuthenticationEvent(status="authenticating", username=None)
    await ui.on_authentication_event(event1)

    # Test authenticated
    event2 = AuthenticationEvent(status="authenticated", username="testuser")
    await ui.on_authentication_event(event2)

    # Test failed
    event3 = AuthenticationEvent(status="failed", username=None)
...
async def test_processing_summary_event_handler(config, event_bus):
    ...
    event = ProcessingSummaryEvent(
        ...
        failed=[("game6.nes", "Network error"), ("game7.nes", "No match found")],
    )
...
async def test_search_queue_processing(config, event_bus):
    ...
    search_results = [
        {"game_data": {"id": "123", "name": "Test", "region": "USA"}, "confidence": 0.9}
    ]
...
async def test_cumulative_metadata_tracking(config, event_bus):
    ...
    event1 = APIActivityEvent(
        metadata_in_flight=5, metadata_total=10, search_in_flight=0, search_total=0
    )
    ...
    event2 = APIActivityEvent(
        metadata_in_flight=3, metadata_total=12, search_in_flight=0, search_total=0
    )
...
async def test_event_type_validation(config, event_bus):
    ...
    correct_event = CacheMetricsEvent(
        existing=100, added=10, hits=90, misses=20, hit_rate=81.8
    )
