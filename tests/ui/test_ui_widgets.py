...
async def test_ui_state_consistency(config, event_bus):
    ...
    # 1. System starts
    await ui.on_system_started(SystemStartedEvent("nes", "NES", 100, 0, 3))
    assert ui.current_system is not None

    # 2. Hashing progresses
    await ui.on_hashing_progress(HashingProgressEvent(50, 100, True, 5))

    # 3. API activity occurs
    await ui.on_api_activity(APIActivityEvent(3, 50, 1, 10))

    # 4. Media downloads
    await ui.on_media_download(MediaDownloadEvent("box-2D", "game.nes", "complete"))

    # 5. Game completes
    await ui.on_game_completed(GameCompletedEvent("123", "Test Game", "2024"))

    # 6. System completes
    await ui.on_system_completed(SystemCompletedEvent("nes", 100, 85, 5, 10, 120.0))
...
async def test_active_requests_table_duration_updates():
    ...
    async with TestApp().run_test() as pilot:
        ...
        # Add another request
        widget.update_request(
            rom_name="Test ROM 2.nes", stage="Media DL", status="Active"
        )
